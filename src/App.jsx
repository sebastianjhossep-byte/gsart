...
const SAMPLE_PRODUCTS = [
  { id: "1", name: "Abstracción Nocturna", description: "Óleo sobre lienzo.", price: 1200, stock: 3, category: "Óleo", images: [], is_boutique: false, active: true },
  { id: "2", name: "Geometría del Alma", description: "Acrílico sobre madera.", price: 850, stock: 5, category: "Acrílico", images: [], is_boutique: false, active: true },
  { id: "3", name: "Fragmentos de Luz", description: "Técnica mixta.", price: 650, stock: 8, category: "Técnica Mixta", images: [], is_boutique: false, active: true },
  { id: "4", name: "Retrato del Silencio", description: "Carboncillo.", price: 2100, stock: 2, category: "Dibujo", images: [], is_boutique: false, active: true },
  { id: "5", name: "Colección Éter — Pieza I", description: "Exclusivo.", price: 4500, stock: 1, category: "Boutique", images: [], is_boutique: true, active: true },
  { id: "6", name: "Colección Éter — Pieza II", description: "Edición única.", price: 3800, stock: 1, category: "Boutique", images: [], is_boutique: true, active: true },
];
...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root {
      --black: #060608; --dark: #0e0d14; --dark2: #16141f; --dark3: #1e1b2a;
      --border: rgba(155,79,204,0.15); --border2: rgba(155,79,204,0.3);
      --text: #e8e2f0; --text2: #8a7fa0;
      --purple: #9b4fcc; --purple2: #bf7ef0; --purple3: #6b2fa0;
      --purple-glow: rgba(155,79,204,0.2); --red: #c0392b; --green: #27ae60; --white: #f0ecf8;
    }
    html { scroll-behavior: smooth; }
    body { font-family: 'Montserrat', sans-serif; background: var(--black); color: var(--text); font-size: 13px; line-height: 1.7; letter-spacing: 0.02em; min-height: 100vh; }
    button { font-family: inherit; cursor: pointer; border: none; outline: none; }
    input, textarea, select { font-family: inherit; outline: none; }
    a { text-decoration: none; color: inherit; }
    ::-webkit-scrollbar { width: 2px; } ::-webkit-scrollbar-track { background: var(--black); } ::-webkit-scrollbar-thumb { background: var(--purple); border-radius: 2px; }
    ...
    .purple-shimmer { background: linear-gradient(90deg, #6b2fa0 0%, #bf7ef0 40%, #9b4fcc 60%, #6b2fa0 100%); background-size: 200% auto; -webkit-background-clip: text; -webkit-text-fill-color: transparent; animation: shimmer 3s linear infinite; }
...
const Btn = ({ children, variant = "purple", size = "md", onClick, style = {}, disabled }) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled} style={{ ...v[variant], ...s[size], borderRadius: 0, fontWeight: 500, textTransform: "uppercase", transition: "all 0.3s ease", opacity: disabled ? 0.4 : 1, display: "inline-flex", alignItems: "center", gap: 8, ...style }}
...
const Input = ({ label, error, ...props }) => (
  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
    ...
    <input {...props} style={{ padding: "12px 0", borderRadius: 0, border: "none", borderBottom: `1px solid ${error ? "var(--red)" : "var(--border2)"}`, background: "transparent", color: "var(--text)", fontSize: 13, transition: "border-color 0.3s", ...props.style }}
...
const ArtworkPlaceholder = ({ name, category, isBoutique, size = "md" }) => {
  ...
  return (
    <div style={{ width: "100%", height: sizes[size], background: patterns[name.length % patterns.length], display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", border: "1px solid var(--border)" }}>
      {isBoutique && <div style={{ position: "absolute", top: 12, right: 12, fontSize: 8, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--purple2)", border: "1px solid var(--purple)", padding: "3px 8px" }}>Exclusivo</div>}
...
const Navbar = ({ user, cartCount, onCart, onAuth, onLogout, onAdmin, onBoutique, currentPage, setPage }) => {
  ...
  const navLinkStyle = { background: "none", border: "none", fontSize: 14, letterSpacing: "0.25em", textTransform: "uppercase", color: "var(--text2)", cursor: "pointer", transition: "color 0.3s", padding: "8px 0" };
  return (
    <>
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 300, padding: scrolled ? "14px 20px" : "24px 20px", background: scrolled ? "rgba(6,6,8,0.96)" : "transparent", borderBottom: scrolled ? "1px solid var(--border)" : "none", display: "flex", alignItems: "center", justifyContent: "space-between", transition: "all 0.4s ease", backdropFilter: scrolled ? "blur(20px)" : "none" }}>
        ...
        <div className="nav-links" style={{ display: "flex", gap: 32, alignItems: "center" }}>
          {[["Colección","collection"],["Artistas","artists"],["Lanzamientos","launches"],["Boutique","boutique"]].map(([label, pg]) => (
            <button key={pg} onClick={() => pg === "boutique" ? onBoutique() : setPage(pg)} style={{ background: "none", border: "none", fontSize: 9, letterSpacing: "0.25em", textTransform: "uppercase", color: currentPage === pg ? "var(--purple2)" : "var(--text2)", cursor: "pointer", transition: "color 0.3s", borderBottom: pg === "boutique" ? "1px solid var(--purple)" : "none", paddingBottom: pg === "boutique" ? 2 : 0 }}
        ...
        <div style={{ display: "flex", gap: 16, alignItems: "center", zIndex: 301 }}>
          ...
          {user ? <button onClick={onLogout} style={{ background: "none", border: "none", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--text2)", cursor: "pointer" }}>Salir</button>
      ...
      {menuOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(6,6,8,0.98)", zIndex: 200, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 40 }}>
          ...
          {[["Colección","collection"],["Artistas","artists"],["Lanzamientos","launches"],["Boutique","boutique"]].map(([label, pg]) => (
            <button key={pg} onClick={() => { pg === "boutique" ? onBoutique() : setPage(pg); setMenuOpen(false); }} style={{ ...navLinkStyle, fontSize: 16, color: "var(--text)" }}>{label}</button>
          ))}
          {user ? <button onClick={() => { onLogout(); setMenuOpen(false); }} style={{ ...navLinkStyle, color: "var(--text2)" }}>Salir</button>
...
const Hero = ({ setPage, onBoutique }) => (
  <div className="hero-padding" style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", textAlign: "center", padding: "0 40px" }}>
    ...
    <div style={{ position: "relative", zIndex: 1 }} className="fade-in">
      <div style={{ marginBottom: 32, display: "flex", justifyContent: "center" }}><img src="/gsartLOGO.png" alt="GSART" className="logo-glow" style={{ height: 80, width: 80, objectFit: "contain" }} /></div>
      ...
      <p style={{ maxWidth: 480, margin: "0 auto 40px", color: "var(--text2)", fontSize: 14, fontWeight: 300, lineHeight: 1.9, fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic" }}>Cuadros únicos de artistas emergentes. Cada obra es una conversación entre el artista y el coleccionista.</p>
...
const ProductCard = ({ product, onAddToCart, onClick }) => {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={{ cursor: "pointer", transition: "transform 0.4s ease", transform: hovered ? "translateY(-8px)" : "translateY(0)" }}>
      ...
      <button onClick={() => onAddToCart(product)} style={{ marginTop: 16, width: "100%", padding: "12px", background: "transparent", border: "1px solid var(--border2)", color: "var(--text2)", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", cursor: "pointer", transition: "all 0.3s" }}
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ marginBottom: 48 }}>
          ...
          <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
            {categories.map(cat => <button key={cat} onClick={() => setFilter(cat)} style={{ background: "none", border: "none", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", cursor: "pointer", padding: "0 0 6px", color: filter === cat ? "var(--purple2)" : "var(--text2)", borderBottom: filter === cat ? "1px solid var(--purple)" : "1px solid transparent", transition: "all 0.3s" }}>{cat}</button>)}
          </div>
        </div>
...
const Countdown = ({ targetDate }) => {
  ...
  if (new Date(targetDate) <= new Date()) return <div style={{ fontSize: 11, letterSpacing: "0.3em", textTransform: "uppercase", color: "var(--green)" }}>¡Ya disponible!</div>;
  return (
    <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
      {[["días", time.days], ["hrs", time.hours], ["min", time.minutes], ["seg", time.seconds]].map(([label, val]) => (
        <div key={label} style={{ textAlign: "center" }}>
          <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 24, fontWeight: 300, color: "var(--purple2)", lineHeight: 1 }}>{pad(val)}</div>
...
const LaunchesPage = () => {
  ...
  const sendNotification = async () => {
    ...
    await supabase.from("launch_notifications").insert({ launch_id: notifLaunch.id, email: notifEmail }).catch(() => {});
    setNotifSent(true);
  };
  return (
    <div className="collection-padding" style={{ padding: "120px 48px 80px" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        {launches.length === 0 ? (
          <div style={{ textAlign: "center", padding: "80px 0" }}>
            <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 24, fontStyle: "italic", color: "var(--text2)" }}>Próximamente nuevas obras...</div>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {launches.map((launch, i) => {
              ...
              return (
                <div key={launch.id} className="fade-in" style={{ display: "grid", gridTemplateColumns: "60px 1fr auto", gap: 24, alignItems: "center", padding: "28px 20px", background: i % 2 === 0 ? "var(--dark)" : "transparent", border: "1px solid var(--border)", transition: "border-color 0.3s" }}
                  onMouseEnter={e => e.currentTarget.style.borderColor = "var(--border2)"}
                  onMouseLeave={e => e.currentTarget.style.borderColor = "var(--border)"}
                >
                  <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 40, fontWeight: 300, color: "rgba(155,79,204,0.2)", lineHeight: 1, textAlign: "center" }}>{String(i + 1).padStart(2, "0")}</div>
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6, flexWrap: "wrap" }}>
                      {launch.category && <span style={{ fontSize: 8, letterSpacing: "0.3em", textTransform: "uppercase", color: "var(--purple)", border: "1px solid var(--border2)", padding: "2px 8px" }}>{launch.category}</span>}
                      {isPast && <span style={{ fontSize: 8, letterSpacing: "0.3em", textTransform: "uppercase", color: "var(--green)", border: "1px solid rgba(39,174,96,0.3)", padding: "2px 8px" }}>Disponible</span>}
                    </div>
                    <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, color: "var(--white)", marginBottom: 6 }}>{launch.title}</div>
                    {launch.description && <div style={{ fontSize: 12, color: "var(--text2)", lineHeight: 1.7, marginBottom: 8, maxWidth: 500 }}>{launch.description}</div>}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 12, minWidth: 140 }}>
                    ...
                    {!isPast && (
                      <button onClick={() => { setNotifLaunch(launch); setNotifEmail(""); setNotifSent(false); }} style={{ background: "transparent", border: "1px solid var(--border2)", color: "var(--text2)", fontSize: 8, letterSpacing: "0.2em", textTransform: "uppercase", padding: "6px 12px", cursor: "pointer", transition: "all 0.3s" }}
      ...
      {notifLaunch && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(6,6,8,0.95)", zIndex: 900, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
          ...
          <div className="slide-up" style={{ background: "var(--dark)", border: "1px solid var(--border2)", width: "100%", maxWidth: 420, padding: 48, position: "relative", textAlign: "center" }}>
            ...
            <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 24, fontWeight: 300, marginBottom: 8 }}>{notifLaunch.title}</h3>
            <p style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic", color: "var(--text2)", fontSize: 13, marginBottom: 28 }}>Te avisaremos cuando esta obra esté disponible.</p>
            {!notifSent ? (
              <>
                <Input label="Tu email" type="email" placeholder="tu@email.com" value={notifEmail} onChange={e => setNotifEmail(e.target.value)} />
                <Btn size="lg" onClick={sendNotification} disabled={!notifEmail} style={{ marginTop: 20 }}>Notificarme</Btn>
              </>
            ) : (
              <div>
                ...
                <div style={{ fontSize: 12, color: "var(--text2)" }}>Te avisaremos en {notifEmail}</div>
              </div>
            )}
          </div>
        </div>
...
const BoutiqueModal = ({ onClose, onAccess, products }) => {
  const [code, setCode] = useState(""); const [error, setError] = useState(""); const [loading, setLoading] = useState(false); const [unlocked, setUnlocked] = useState(false);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(6,6,8,0.97)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="slide-up" style={{ background: "var(--dark)", border: "1px solid var(--border2)", width: "100%", maxWidth: unlocked ? 900 : 480, maxHeight: "90vh", overflowY: "auto", position: "relative" }}>
        {!unlocked ? (
          <div className="modal-padding" style={{ padding: 60, textAlign: "center" }}>
            ...
            <p style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic", color: "var(--text2)", fontSize: 15, marginBottom: 40, lineHeight: 1.8 }}>Un espacio reservado para coleccionistas seleccionados.<br />Introduce tu código de acceso para descubrir piezas únicas.</p>
            ...
            <Btn onClick={handleSubmit} disabled={loading || !code.trim()} size="lg" style={{ width: "auto", margin: "0 auto" }}>{loading ? "Verificando..." : "Acceder"}</Btn>
            <div style={{ marginTop: 32, paddingTop: 32, borderTop: "1px solid var(--border)", fontSize: 10, color: "var(--text2)" }}>¿No tienes código? Contacta con nosotros para ser invitado.</div>
          </div>
        ) : (
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 400 }} onClick={e => e.target === e.currentTarget && onClose()}>
      ...
      <div className="slide-up cart-width" style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: "100%", maxWidth: 480, background: "var(--dark)", borderLeft: "1px solid var(--border2)", display: "flex", flexDirection: "column" }}>
        <div style={{ padding: "24px 24px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div><div style={{ fontSize: 9, letterSpacing: "0.4em", textTransform: "uppercase", color: "var(--purple)", marginBottom: 4 }}>Tu selección</div><h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 26, fontWeight: 300 }}>Carrito</h3></div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: "var(--text2)", fontSize: 24, cursor: "pointer" }}>×</button>
        </div>
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
          {cart.length === 0 ? <div style={{ textAlign: "center", padding: "60px 0", color: "var(--text2)" }}><div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, fontStyle: "italic" }}>Tu carrito está vacío</div></div>
            : cart.map((item, i) => (
        ...
        {cart.length > 0 && (
          <div style={{ padding: "20px 24px 32px", borderTop: "1px solid var(--border)" }}>
            ...
            {user ? <Btn size="lg" onClick={onCheckout}>Proceder al pago</Btn> : <div><Btn size="lg" onClick={onAuth}>Acceder para comprar</Btn><div style={{ fontSize: 10, color: "var(--text2)", textAlign: "center", marginTop: 12 }}>Necesitas una cuenta para finalizar tu compra</div></div>}
          </div>
        )}
      </div>
    </div>
...
const CheckoutModal = ({ cart, user, onClose, onSuccess }) => {
  const [step, setStep] = useState(1); const [payMethod, setPayMethod] = useState("stripe"); const [loading, setLoading] = useState(false);
  const [address, setAddress] = useState({ street: "", city: "", zip: "", country: "España" });
  const total = cart.reduce((sum, item) => sum + item.price * item.qty, 0);
  const handleOrder = async () => {
    setLoading(true);
    const { data: order } = await supabase.from("orders").insert({ user_id: user.id, status: "pending", total, payment_method: payMethod, payment_status: "paid", shipping_address: address }).select().single();
    if (order) { await supabase.from("order_items").insert(cart.map(item => ({ order_id: order.id, product_id: item.id, product_name: item.name, quantity: item.qty, price: item.price }))); }
    setLoading(false); setStep(3);
  };
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(6,6,8,0.97)", zIndex: 600, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="slide-up checkout-padding" style={{ background: "var(--dark)", border: "1px solid var(--border2)", width: "100%", maxWidth: 560, padding: 48, position: "relative", maxHeight: "95vh", overflowY: "auto" }}>
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 12, marginBottom: 32 }}>
              {[["stripe", "💳 Tarjeta"], ["paypal", "🅿 PayPal"]].map(([id, label]) => (
                <button key={id} onClick={() => setPayMethod(id)} style={{ flex: 1, padding: "14px", background: "transparent", cursor: "pointer", border: `1px solid ${payMethod === id ? "var(--purple)" : "var(--border)"}`, color: payMethod === id ? "var(--purple2)" : "var(--text2)", fontSize: 12, transition: "all 0.3s" }}>{label}</button>
              ))}
            </div>
        ...
        {step === 3 && (
          <div style={{ textAlign: "center", padding: "20px 0" }}>
            ...
            <p style={{ fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic", color: "var(--text2)", fontSize: 14, lineHeight: 1.8, marginBottom: 32 }}>Su obra será preparada con el mayor cuidado y enviada en los próximos días.</p>
...
const AuthModal = ({ onClose, onAuth }) => {
  const [mode, setMode] = useState("login"); const [email, setEmail] = useState(""); const [password, setPassword] = useState(""); const [name, setName] = useState(""); const [loading, setLoading] = useState(false); const [error, setError] = useState(""); const [success, setSuccess] = useState("");
  const handleSubmit = async () => {
    setError(""); setSuccess("");
    ...
    if (mode === "login") {
      ...
      if (err) setError("Email o contraseña incorrectos"); else { onAuth(data.user); onClose(); }
    } else {
      ...
      if (err) setError(err.message); else setSuccess("¡Cuenta creada! Ya puedes iniciar sesión.");
    }
    setLoading(false);
  };
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(6,6,8,0.97)", zIndex: 700, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="slide-up" style={{ background: "var(--dark)", border: "1px solid var(--border2)", width: "100%", maxWidth: 440, padding: "40px 28px", position: "relative" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <div style={{ fontSize: 9, letterSpacing: "0.4em", textTransform: "uppercase", color: "var(--text2)" }}>{mode === "login" ? "Bienvenido de nuevo" : "Únete a la galería"}</div>
        </div>
        <div style={{ display: "flex", marginBottom: 28, borderBottom: "1px solid var(--border)" }}>
          {[["login", "Acceder"], ["register", "Registrarse"]].map(([m, l]) => (
            <button key={m} onClick={() => { setMode(m); setError(""); setSuccess(""); }} style={{ flex: 1, padding: "12px", background: "none", border: "none", fontSize: 9, letterSpacing: "0.2em", textTransform: "uppercase", cursor: "pointer", color: mode === m ? "var(--purple2)" : "var(--text2)", borderBottom: mode === m ? "1px solid var(--purple)" : "none", marginBottom: -1, transition: "color 0.3s" }}>{l}</button>
          ))}
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <Btn size="lg" onClick={handleSubmit} disabled={loading} style={{ marginTop: 8 }}>{loading ? "..." : mode === "login" ? "Acceder" : "Crear cuenta"}</Btn>
        </div>
      </div>
    </div>
  );
...
const AdminPanel = ({ onClose }) => {
  ...
  const [launches, setLaunches] = useState([]);
  const [newCode, setNewCode] = useState(""); const [newCodeEmail, setNewCodeEmail] = useState("");
  ...
  const createCode = async () => { const code = newCode || generateCode(); const { data } = await supabase.from("boutique_codes").insert({ code, email: newCodeEmail || null }).select().single(); if (data) { setCodes(prev => [data, ...prev]); setNewCode(""); setNewCodeEmail(""); } };
  const createProduct = async () => { const { data } = await supabase.from("products").insert({ ...newProduct, price: parseFloat(newProduct.price), stock: parseInt(newProduct.stock), images: [] }).select().single(); if (data) { setProducts(prev => [data, ...prev]); setShowNewProduct(false); setNewProduct({ name: "", description: "", price: "", stock: "", category: "", is_boutique: false }); } };
  const toggleActive = async (product) => { const { data } = await supabase.from("products").update({ active: !product.active }).eq("id", product.id).select().single(); if (data) setProducts(prev => prev.map(p => p.id === product.id ? data : p)); };

  const createLaunch = async () => {
    const { data } = await supabase.from("launches").insert({ ...newLaunch, active: true }).select().single();
    if (data) { setLaunches(prev => [...prev, data].sort((a, b) => new Date(a.launch_date) - new Date(b.launch_date))); setNewLaunch({ title: "", description: "", launch_date: "", category: "" }); }
  };
  const updateLaunch = async () => {
    const { data } = await supabase.from("launches").update({ title: editingLaunch.title, description: editingLaunch.description, launch_date: editingLaunch.launch_date, category: editingLaunch.category }).eq("id", editingLaunch.id).select().single();
    if (data) { setLaunches(prev => prev.map(l => l.id === data.id ? data : l).sort((a, b) => new Date(a.launch_date) - new Date(b.launch_date))); setEditingLaunch(null); }
  };
  const deleteLaunch = async (id) => { await supabase.from("launches").update({ active: false }).eq("id", id); setLaunches(prev => prev.filter(l => l.id !== id)); };
  const toLocalDatetime = (iso) => { if (!iso) return ""; const d = new Date(iso); const pad = n => String(n).padStart(2, "0"); return `${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}T${pad(d.getHours())}:${pad(d.getMinutes())}`; };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(6,6,8,0.99)", zIndex: 800, overflowY: "auto" }}>
      <div className="admin-padding" style={{ maxWidth: 1100, margin: "0 auto", padding: "48px 40px" }}>
        ...
        {tab === "products" && (
          <div>
            <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 20 }}><Btn onClick={() => setShowNewProduct(!showNewProduct)}>+ Nuevo producto</Btn></div>
            {showNewProduct && (
              <div style={{ background: "var(--dark)", border: "1px solid var(--border2)", padding: "24px 20px", marginBottom: 28 }}>
                ...
                <div style={{ marginBottom: 16 }}><Input label="Descripción" value={newProduct.description} onChange={e => setNewProduct(p => ({ ...p, description: e.target.value }))} /></div>
              ...
              {products.map(p => (
                <div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", background: "var(--dark)", border: "1px solid var(--border)", opacity: p.active ? 1 : 0.4, flexWrap: "wrap", gap: 8 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 9, color: "var(--text2)", letterSpacing: "0.15em", textTransform: "uppercase", marginTop: 2 }}>{p.category} · {p.price?.toLocaleString("es-ES")} € · Stock: {p.stock}{p.is_boutique && <span style={{ color: "var(--purple2)", marginLeft: 8 }}>✦ Boutique</span>}</div>
                  </div>
        ...
        {tab === "orders" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
            {orders.length === 0 ? <div style={{ textAlign: "center", padding: "60px 0", color: "var(--text2)", fontFamily: "'Cormorant Garamond', serif", fontSize: 22, fontStyle: "italic" }}>No hay pedidos aún</div>
              : orders.map(order => (
                    ...
                    <div style={{ textAlign: "right" }}>
                      ...
                      <div style={{ display: "flex", gap: 8, alignItems: "center", justifyContent: "flex-end" }}>
                        ...
                        {order.status !== "cancelled" && (
                          <button onClick={async () => { await supabase.from("orders").update({ status: "cancelled" }).eq("id", order.id); setOrders(prev => prev.map(o => o.id === order.id ? { ...o, status: "cancelled" } : o)); }} style={{ background: "none", border: "1px solid rgba(192,57,43,0.4)", color: "var(--red)", fontSize: 8, letterSpacing: "0.1em", textTransform: "uppercase", padding: "3px 8px", cursor: "pointer" }}>Cancelar</button>
                        )}
                      </div>
                    </div>
                  </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {codes.map(c => (
                <div key={c.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", background: "var(--dark)", border: "1px solid var(--border)", opacity: c.used ? 0.4 : 1, flexWrap: "wrap", gap: 8 }}>
                  ...
                  <div style={{ fontSize: 9, letterSpacing: "0.15em", textTransform: "uppercase", color: c.used ? "var(--red)" : "var(--green)" }}>{c.used ? `Usado ${new Date(c.used_at).toLocaleDateString("es-ES")}` : "Disponible"}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {tab === "launches" && (
          <div>
            ...
            <div style={{ background: "var(--dark)", border: "1px solid var(--border2)", padding: "24px 20px", marginBottom: 28 }}>
              ...
              <div style={{ marginBottom: 16 }}><Input label="Descripción" value={newLaunch.description} onChange={e => setNewLaunch(p => ({ ...p, description: e.target.value }))} placeholder="Descripción de la obra..." /></div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              ...
              {launches.map(launch => (
                <div key={launch.id}>
                  {editingLaunch?.id === launch.id ? (
                    <div style={{ background: "var(--dark)", border: "1px solid var(--purple)", padding: "20px 16px", marginBottom: 2 }}>
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 16, marginBottom: 16 }}>
                        <Input label="Título" value={editingLaunch.title} onChange={e => setEditingLaunch(p => ({ ...p, title: e.target.value }))} />
                        <Input label="Categoría" value={editingLaunch.category || ""} onChange={e => setEditingLaunch(p => ({ ...p, category: e.target.value }))} />
                        <Input label="Fecha" type="datetime-local" value={toLocalDatetime(editingLaunch.launch_date)} onChange={e => setEditingLaunch(p => ({ ...p, launch_date: e.target.value }))} />
                      </div>
                      <div style={{ marginBottom: 16 }}><Input label="Descripción" value={editingLaunch.description || ""} onChange={e => setEditingLaunch(p => ({ ...p, description: e.target.value }))} /></div>
                      <div style={{ display: "flex", gap: 12 }}>
                        <Btn size="sm" onClick={updateLaunch}>Guardar cambios</Btn>
                        <Btn size="sm" variant="ghost" onClick={() => setEditingLaunch(null)}>Cancelar</Btn>
                      </div>
                    </div>
                  ) : (
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", background: "var(--dark)", border: "1px solid var(--border)", flexWrap: "wrap", gap: 8 }}>
                      <div>
                        <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 17, color: "var(--white)" }}>{launch.title}</div>
                        <div style={{ fontSize: 9, color: "var(--text2)", letterSpacing: "0.15em", textTransform: "uppercase", marginTop: 2 }}>
                          {launch.category && `${launch.category} · `}
                          {new Date(launch.launch_date).toLocaleDateString("es-ES", { day: "numeric", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" })}
                        </div>
                      </div>
                      <div style={{ display: "flex", gap: 8 }}>
                        <Btn size="sm" variant="outline" onClick={() => setEditingLaunch(launch)}>Editar</Btn>
                        <Btn size="sm" variant="danger" onClick={() => deleteLaunch(launch.id)}>Eliminar</Btn>
                      </div>
                    </div>
                  )}
                </div>
...
const Footer = ({ setPage }) => (
  <footer className="footer-grid" style={{ borderTop: "1px solid var(--border)", padding: "60px 48px", display: "grid", gridTemplateColumns: "2fr 1fr 1fr", gap: 40 }}>
    <div>
      ...
      <p style={{ fontSize: 12, color: "var(--text2)", lineHeight: 1.8, maxWidth: 280, fontFamily: "'Cormorant Garamond', serif", fontStyle: "italic" }}>Simplemente Estilo Arte. Cuadros únicos de artistas emergentes para coleccionistas exigentes.</p>
    </div>
    <div>
      <div style={{ fontSize: 9, letterSpacing: "0.3em", textTransform: "uppercase", color: "var(--purple)", marginBottom: 20 }}>Galería</div>
      {[["Colección","collection"],["Artistas","artists"],["Lanzamientos","launches"]].map(([l, p]) => (
        <div key={p} style={{ marginBottom: 12 }}>
          <button onClick={() => setPage(p)} style={{ background: "none", border: "none", fontSize: 11, color: "var(--text2)", cursor: "pointer" }} onMouseEnter={e => e.currentTarget.style.color = "var(--purple2)"} onMouseLeave={e => e.currentTarget.style.color = "var(--text2)"}>{l}</button>
        </div>
      ))}
    </div>
    <div>
      <div style={{ fontSize: 9, letterSpacing: "0.3em", textTransform: "uppercase", color: "var(--purple)", marginBottom: 20 }}>Contacto</div>
      <div style={{ fontSize: 11, color: "var(--text2)", lineHeight: 2 }}><div>info@gsart.com</div><div>+34 600 000 000</div><div>Madrid, España</div></div>
    </div>
...
export default function App() {
  ...
  const [products, setProducts] = useState(SAMPLE_PRODUCTS); const [cart, setCart] = useState([]);
  const [showCart, setShowCart] = useState(false); const [showAuth, setShowAuth] = useState(false);
  const [showBoutique, setShowBoutique] = useState(false); const [showCheckout, setShowCheckout] = useState(false);
  const [showAdmin, setShowAdmin] = useState(false); const [selectedProduct, setSelectedProduct] = useState(null);
  ...
  const addToCart = (product) => {
    setCart(prev => { const existing = prev.findIndex(i => i.id === product.id); if (existing >= 0) { const u = [...prev]; u[existing] = { ...u[existing], qty: u[existing].qty + 1 }; return u; } return [...prev, { ...product, qty: 1 }]; });
    setShowCart(true);
  };
